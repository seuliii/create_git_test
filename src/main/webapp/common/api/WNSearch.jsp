		...
		*/
		...
		public String getField(String collectionName, String field, int idx, boolean isHL) {
			...
			if (field.toUpperCase().equals("UID")) {
				...
				ret = parseDate(common.getField(collectionName, field ,idx),"yyyyMMddHHmmss","yyyy.MM.dd");
		...
		*/
		 ...
		 */
		 public String getPageLinks(int startCount, int totalCount, int viewListCount, int bundleCount) {
				...
				if(wnanchor.getCurPageNumber() != 1) { //첫 페이지가 아닐 떄
					sbRet.append("<li><a href=\"#none\" class=\"first\" title=\"first Page\" onClick=\"javascript:doPaging('0');\"> <span class=\"blind\">첫페이지 이동</span></a></li>");
					sbRet.append("<li><a href=\"#none\" class=\"prev\" title=\"Previous Page\"  onClick=\"javascript:doPaging('"+ wnanchor.getBefore()  +"');\"><span class=\"blind\">이전페이지 이동</span></a></li>");

				} else { //첫 페이지
					if(pageCount != 1){
						sbRet.append("<li><a href=\"#none\" class=\"first\" title=\"first Page\"><span class=\"blind\">첫페이지 이동</span></a></li>");
						sbRet.append("<li><a href=\"#none\" class=\"prev\"  title=\"Previous Page\"><span class=\"blind\">이전페이지 이동</span></a></li>");
					}
				} 
	 
				for(int i=0; i<pageCount && i < pages.length; i++) {
					/* if(pageCount == 1){
						sbRet.append("<li class=\"active\"><a href=\"#none\" title=\"현재 페이지\">" + pages[i][0] + "</a></li>");
					} */
					if(pages[i][1].equals("-1")) {
						sbRet.append("<li class=\"active\"><a href=\"#none\" title=\"현재 페이지\">" + pages[i][0] + "</a></li>");
					} else {
						sbRet.append("<li><a href=\"#none\" onClick=\"javascript:doPaging('"+pages[i][1]+"');\" > "+pages[i][0]+" </a></li>");
					}
				
				}
				if(wnanchor.getCurPageNumber() != wnanchor.getLastPage() && wnanchor.getNext() != -1) {
					
					sbRet.append("<li><a href=\"#none\" class=\"next\" onClick=\"javascript:doPaging('"+wnanchor.getNext()+"')\" title=\"Next Page\"><span class=\"blind\">다음페이지 이동</span></a></li>");
					sbRet.append("<li><a href=\"#none\" class=\"last\"  onClick=\"javascript:doPaging('"+wnanchor.getLastPage()+"')\" title=\"last Page\"><span class=\"blind\">마지막페이지 이동</span></a></li>");

				} else {
					if(pageCount != 1){
						sbRet.append("<li><a href=\"#none\" class=\"next\" title=\"Next Page\"><span class=\"blind\">다음페이지 이동</span></a></li>");
						sbRet.append("<li><a href=\"#none\" class=\"last\" title=\"last Page\"><span class=\"blind\">마지막페이지 이동</span></a></li>");
					}
				}
		...
		public String getResultField(String col){
			String documents = null;
			for(int idx = 0; idx < COLLECTIONS.length; idx++){
				if(COLLECTIONS[idx].equals(col)){
					return wncol.COLLECTION_INFO[idx][RESULT_FIELD];
				}
			}
			return null;
		}
		
		public String getResultFieldName(String document){
			
				if(document.indexOf("/") == -1)	{
					return document;	
				}else{
/* 					System.out.println("search=======> " + document.substring(0, document.indexOf("/"))); */
					return document.substring(0, document.indexOf("/"));
				}
		}
		
		public String getSearchField(String col){
			String documents = null;
			for(int idx = 0; idx < COLLECTIONS.length; idx++){
				if(COLLECTIONS[idx].equals(col)){
					return wncol.COLLECTION_INFO[idx][SEARCH_FIELD];
				}
			}
			return null;
		}
